...
int vhx_init_bytes_ready_allreduce(int my_rank, vhx_module_t * module, size_t chunks, int pvt_seq) {
...
int vhx_reduce_internal_cico(const void * sbuf, void * rbuf,
  ...
  vhx_init_bytes_ready_allreduce(rank, vhx_module, chunks, pvt_seq);
  for (int i = 0; i < hier_size; i++) {
    vhx_hier_group_t * hier_group = & (vhx_module -> hier_groups[i]);
    ...
    if (rank == hier_group -> leader) {
      ...
      WAIT_FLAG( & hier_group -> members_shared_ctrl_vars[rank].member_ack, pvt_seq);
    }
...
int vhx_check_members_for_completion(int rank, vhx_hier_group_t * hier_group, int value, int pvt_seq) {
  ...
  for (int j = 0; j < group_size; j++) {
    ...
    if (is_completed < value || (hier_group -> members_shared_ctrl_vars[hier_group -> real_members[j]].member_seq < pvt_seq)) {
      all_completed = 0;
      break;
    }
...
int vhx_reduce_internal_opt2(const void * sbuf, void * rbuf,
  ...
  while (reduced_chunks < chunks + has_remainder) {
    ...
    for (int i = 0; i < hier_size; i++) {
      ...
      if (rank == hier_group -> leader) {
        if (i == 0)
          vhx_set_bytes_ready_allreduce(rank, & vhx_module -> hier_groups[0], chunks);
        ...
        vhx_calculate_bytes_and_offset(rank, hier_group, opal_min(bytes_total, OMPI_vhx_CHUNK_SIZE), dtype_size, & bytes_of_proc, & uniform_offset);
        (vhx_module -> hier_groups[i].members_shared_ctrl_vars[rank].member_seq) = pvt_seq;
        ...
        for (int j = 0; j < hier_group -> real_size; j++) {
          ...
          do {
            ...
            chunks_ready = __atomic_load_n( & (hier_group -> shared_ctrl_vars[hier_group -> leader].reduce_ready_chunks), __ATOMIC_RELAXED);
          }
          ...
          if (OMPI_vhx_CHUNK_SIZE) {
            ...
            reduce_count = (reduced_chunks == chunks && has_remainder) ? remainder : opal_min(bytes_of_proc, bytes_total) / dtype_size;
          }
...
int vhx_reduce_internal_xpmem(const void * sbuf, void * rbuf,
  ...
  int remainder = (OMPI_vhx_CHUNK_SIZE && OMPI_vhx_CHUNK_SIZE < bytes_total) ? bytes_total % OMPI_vhx_CHUNK_SIZE : 0;
  int has_remainder = remainder ? 1 : 0;
  vhx_init_bytes_ready_allreduce(rank, vhx_module, 0, pvt_seq);
  int complete_levels = 0;
  int first_reduction_done = 0;
  int reduced_chunks = 0;
  int sbuf_memcpy = 0;
